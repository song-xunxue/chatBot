...
2026-06-24
变更说明：
  1. M4.1 创建 Plugin ABC + PluginContext + HookResult + 钩子名常量
"""
...
ON_TICK = "on_tick"                   # 定时（cron-like，M4.4 调度器驱动）
ON_DELETE = "on_delete"               # 消息标记删除（persona_evolve 负样本）
...
class PluginContext:
    ...
    # 插件自身运行时状态建议存在 Plugin 实例属性上，而非此处（避免多实例串扰）
