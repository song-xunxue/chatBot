...
- on_tick：把当前时辰标签缓存到 Redis(供 proactive_msg 决定是否问候)
时辰/季节判定为纯函数。datetime 在插件 Python 运行时可用(非工作流沙箱)。
...
2026-06-24
变更说明：
  1. M4.4 创建 time_aware：时辰/季节注入 + 标签缓存
"""
