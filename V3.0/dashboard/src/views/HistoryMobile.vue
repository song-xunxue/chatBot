...
<template>
  <n-space vertical size="medium">
        ...
        <div v-for="m in messages" :key="m.mid"
          ...
          <div class="m-bubble"
            ...
            <n-space v-if="m.status !== 'deleted'" :size="6" align="center" style="margin-top:4px">
              <n-button size="small" type="info" ghost @click="openNote(m.mid, m.score_note)">批注</n-button>
    ...
    <n-modal v-model:show="noteModalShow" preset="dialog" title="批注">
      <n-space vertical size="small">
        <span class="m-hint">多方面解答:为什么给这个评分 + 这句话的暗含意思/特殊习惯用语(用户消息与角色回复均可批注)</span>
        <n-input v-model:value="noteVal" type="textarea" :autosize="{ minRows: 4, maxRows: 8 }"
                 placeholder="例:这句其实是在撒娇;『行吧』=勉强同意;评分理由:语气贴合人设" />
        <span class="m-hint">批注会作为 [批注: ...] 进入人设反推上下文(管理员解读优先采信)</span>
      </n-space>
      <template #action>
        <n-button size="large" @click="noteModalShow = false">取消</n-button>
        <n-button size="large" type="primary" @click="applyNote">确定</n-button>
      </template>
    </n-modal>
    ...
    <n-modal v-model:show="scoreModalShow" preset="dialog" title="改分">
      <n-space vertical size="small">
        ...
        <span class="m-hint">批注(为什么这个分 + 这句话的暗含意思/习惯用语,可选)</span>
