...
class Settings(BaseSettings):
    ...
    # 3.QQ 号(OneBot/NapCat)接入配置(V3.0 纯 QQ 号项目,2026-08-17 精简:官方机器人栈已删,
    #    官方机器人方向的演进在 V2.0 项目独立进行;通用优化双向复用)
    onebot_ws_token: str = Field(default="", validation_alias="ONEBOT_WS_TOKEN")  # 反向WS鉴权(NapCat query access_token / Bearer 头;空=拒连 fail-closed)
    onebot_self_id: str = Field(default="", validation_alias="ONEBOT_SELF_ID")  # 接管对话的 QQ 号(其他连入号事件忽略+不占出站)
    onebot_call_timeout: float = 5.0  # OneBot Action 响应等待秒数(超时软降级)
    # 后台维护循环(mood 衰减/记忆遗忘):V3.0 独立 Redis 后自跑(数据隔离改造后默认开)
    background_maintenance_enable: bool = Field(default=True, validation_alias="BACKGROUND_MAINTENANCE")
